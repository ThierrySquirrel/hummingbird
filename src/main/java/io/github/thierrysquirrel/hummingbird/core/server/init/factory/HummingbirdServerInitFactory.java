...
public class HummingbirdServerInitFactory {
    ...
    public static <T> void init(String url, ServerSocketChannel serverSocketChannel, HummingbirdDomain<T> hummingbirdDomain) {
        for (int i = 0; i < HummingbirdServerInitFactoryConstant.THREADS_NUMBER; i++) {
            ServerSocketSelectorExecution.serverSocketSelector(url, serverSocketChannel, hummingbirdDomain);
        }
    }
}
